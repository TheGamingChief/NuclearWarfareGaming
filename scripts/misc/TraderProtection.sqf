...
diag_log ( "[NWG] Starting Trader City Safezone Commander!" );
 
if ( isDedicated || isServer ) exitWith {diag_log ( "Error: Attempting to start NWG products on a server where it should not be!" );}; 
...
waitUntil {!isNil "player_forceSave"};
if ( AGN_safeZoneMessages ) then { systemChat ( "[NWG] Trader Zone Commander Loaded!" ); };
...
while {true} do {
	...
	if ( AGN_safeZoneMessages ) then { systemChat ("[NWG] Entering Trader Area - God Mode Enabled"); };
	_thePlayer = player;
	...
	if ( AGN_safeZone_Players_DisableWeaponFiring ) then
	{
		_EH_Fired = _thePlayer addEventHandler ["Fired", {
			systemChat ("[NWG] You can not fire your weapon in a Trader City Area");
			...
			while {!canBuild} do
			{
				...
				if( !isNull (FindDisplay 106) && !_skip ) then
				{
					if ( isNull AGN_LastPlayerLookedAt ) then
					{
						...
						if ( AGN_safeZoneMessages ) then { systemChat ("[NWG] Anti Backpack Stealing - Redirecting you to your own gear!"); };
						waitUntil { isNull (FindDisplay 106) };
					} else {
						if ( AGN_safeZoneMessages ) then { systemChat (format["[NWG] You cannot open your gear at this time as you have looked at a player in the last 5 seconds."]); };
						(findDisplay 106) closeDisplay 1;
						waitUntil { isNull (FindDisplay 106) };
					};
				};
	...
	if ( AGN_safeZone_Vehicles_DisableMountedGuns ) then
	{
		while { !canBuild } do
		{
			...
			{
				if (AGN_safeZoneMessages) then { systemChat ( "[NWG] No Firing Vehicle Guns Enabled" ); };
				_inVehicle = vehicle player;
				_inVehicleDamage = getDammage _inVehicle;
				_EH_Fired_Vehicle = _inVehicle addEventHandler ["Fired", {
					systemChat ("[NWG] You can not fire your vehicles weapon in a Trader City Area");
					NearestObject [_this select 0,_this select 4] setPos[0,0,0];
				}];
			};
		};
	...
	if ( AGN_safeZoneMessages ) then { systemChat ("[NWG] Exiting Trader Area - God Mode Disabled"); };
	
	if ( AGN_safeZone_Vehicles_DisableMountedGuns ) then
	{
		if ( !(isNull _inVehicle) ) then
		{
			if ( AGN_safeZoneMessages ) then { systemChat ( "[NWG] No Firing Vehicle Guns Disabled" ); };
			_inVehicle removeEventHandler ["Fired", _EH_Fired_Vehicle];
		};
